...
:: excelEY Setup — one-click install/update/uninstall
:: Downloads sealed excelEY.xlam from GitHub Releases and installs to %APPDATA%\Microsoft\AddIns
:: Registers via HKCU registry so Excel loads it automatically.
:: No admin rights needed.

set "ADDIN=excelEY.xlam"
set "ADDIN_NAME=excelEY"
set "DEST=%APPDATA%\Microsoft\AddIns"
set "DST=%DEST%\%ADDIN%"
set "DOWNLOAD_URL=https://github.com/therepos/excelEY/releases/latest/download/excelEY.xlam"

echo.
echo  ========================================
echo    excelEY - Excel Add-in Setup
echo  ========================================
echo.
...
echo.
echo  Installed. The excelEY tab will appear next time
echo  you open Excel.
echo.
pause
exit /b 0
...
echo  excelEY is already installed at:
echo    %DST%
echo.
...
powershell -NoProfile -Command ^
 ...
 "      New-ItemProperty -Path $k -Name Description -Value 'excelEY Add-in' -PropertyType String -Force | Out-Null "^
 "    } "^
 "  } "^
 "}"
...
powershell -NoProfile -Command ^
 ...
 "    Get-ChildItem $tlBase | ForEach-Object { "^
 "      $p = (Get-ItemProperty $_.PsPath -ErrorAction SilentlyContinue); "^
 "      if ($p.Path -and ($p.Path.TrimEnd('\') + '\') -ieq $tlPath -and $p.Description -like 'excelEY*') { "^
