' ============================================================
' EXTRACT JOBS BY ENGAGEMENT - GANTT/TIMELINE VIEW
' Reads "ER and P&C" sheet and generates a weekly timeline
' showing everyone booked on a specified engagement (job name).
'
' Companion to ExtractJobsByManager: that macro filters by the
' manager shown in parentheses; this one filters by the
' engagement/job name itself, so it surfaces ALL staff booked
' on the engagement regardless of who is managing it.
' ============================================================

Sub ExtractJobsByEngagement()
    ...
    engName = InputBox("Enter engagement (job) name to search for:" & vbCrLf & vbCrLf & _
        "This finds everyone booked on jobs whose name contains" & vbCrLf & _
        "this text, e.g. ""Alpha Audit"", ""FY25 Statutory"", ""ABC"".", _
        "Extract Jobs by Engagement")
    ...
    For R = dataStartRow To lastRow
        ...
        For i = 1 To numAllDates
            ...
            ' Cheap pre-filter: engagement text must appear somewhere in the cell
...
NextRawLine:
            ...
            For j = 1 To numMerged
                ...
                ' --- Total percentage on this booking line ---
                ...
                ' --- Text after the last "%" is "JobName (Manager)" ---
                ...
                ' --- Filter on the ENGAGEMENT name (not the manager/staff) ---
                If InStr(LCase(jobName), searchLower) = 0 Then GoTo NextLine
...
NextRow:
    ...
    If numRes = 0 Then
        MsgBox "No engagements found matching """ & engName & """.", vbInformation
        GoTo Cleanup
    End If
...
NextRes:
    ...
    wsOut.Name = Left("Eng - " & engName, 31)
    On Error GoTo 0

    ' --- Title ---
    wsOut.Range("A1").Value = "Engagement Timeline - " & engName
    ...
    wsOut.Cells(3, 2).Value = "Engagement"
    ...
    For s = 1 To numRows
        ' New engagement group - insert separator
    ...
    MsgBox "Engagement timeline generated for """ & engName & """!" & vbCrLf & _
        uniqueJobs & " engagement(s) matched" & vbCrLf & _
        numRows & " engagement-staff assignments" & vbCrLf & _
